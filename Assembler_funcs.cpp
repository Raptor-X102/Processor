...
void Compile_code(const char* compiling_file, const char* out_file, Labels* labels) {
    ...
    fprintf(stderr, "convertion started\n");
    commands_amount = Convert_txt_to_code(input_file, output_file, file_size, labels);
    fprintf(stderr, "commands_amount = %d", commands_amount);
    printf("ended\n");

    fclose(output_file);
    fprintf(stderr, "input file was closed\n");

    //free(output_file);
    fprintf(stderr, "memory was free\n");
...
uint64_t Convert_txt_to_code(FILE* input_file, FILE* output_file, uint64_t code_length, Labels* labels) {
    ...
    for(uint64_t curr_pos = 0, inword_pos = 0; input_buffer[curr_pos] != EOF &&
        ...
        fprintf(stderr, "input_buffer + curr_pos = %s\n", input_buffer + curr_pos);
        ...
        if(strchr(input_buffer+curr_pos, ':')) {
            ...
            if(!Check_jmp_func(code_str_prev)) {
                ...
                if(label_pos >= 0) {
                    ...
                    fprintf(stderr, "code_str_prev = %s\n", code_str_prev);
                    curr_pos = inword_pos;
    ...
    for(int i = 0; i < curr_code_byte; i++) {
        ...
        if(i % 16 == 0) {
            ...
            for(int j = i; j < i + 16; j++)
                fprintf(stderr, "%p ", code_data + j);

            fprintf(stderr, "\n");

        }
        fprintf(stderr, "      %02X ", code_data[i]);
    }
    fprintf(stderr, "\n");
    getchar();
}
...
int64_t Find_label(const char* label_name, Labels* labels) {
    ...
    fprintf(stderr, "Label was not found\n");
        ...
        if(Chech_brackets(code_str)) {
            ...
            if(push_register >= 0) {
                ...
                *curr_code_byte += 2*sizeof(char);
                ...
                fprintf(stderr, "number = %lg\n", tmp_double);
                fprintf(stderr, "code_data + *commands_amount (%d) = %p\n",
